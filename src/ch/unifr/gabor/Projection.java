...
//		plotArray.show();
	 ...
	 * @return
	 */
	public ArrayList<Integer> findSeparator(int[] projectionData){
		int mean = 0;
		int lastPeak = -1;
		int lastValley = -1;		
		...
		ArrayList<Integer> valleyValues = new ArrayList<Integer>();
		...
		for (int i = 1; i < projectionData.length - 1; i++)
		{
			if (projectionData[i] >= projectionData[i-1] && projectionData[i] >= projectionData[i+1]
				...
				lastPeak = projectionData[i];}
			if (projectionData[i] <= projectionData[i-1] && projectionData[i] <= projectionData[i+1]
					&& projectionData[i] < mean){
				valleys.add(i);
				valleyValues.add(projectionData[i]);
				lastValley = projectionData[i];}
		}
	 ...
	 * @param findPeak
	 */
	public void removeNeighbour(ArrayList<Integer> locations, ArrayList<Integer> values, boolean findPeak){
		boolean clear = true;
		while (true) {
			clear = true;
	...
	public ArrayList<Point> pad(ArrayList<Point> separatingPoints)
	{
		ArrayList<Point> padding = new ArrayList<Point>();
		Point pre = null;
		Point current = null;
		
		LinkedHashMap<Integer, ArrayList<Point>> lines = new LinkedHashMap<Integer, ArrayList<Point>>();
	 ...
	 * @return separatingPoints
	 */
	public ArrayList<Point> showResult(BufferedImage img, ArrayList<Integer> valleys){
		ArrayList<Point> separatingPoints = new ArrayList<Point>();
...
//		pad(separatingPoints);
	 ...
	 */
	public void combineTinyCCs(BufferedImage img, ArrayList<Point> separatingPoints){
...
//			g2d.drawPolygon(adjustedPolygon);
		...
		for (int i = 0; i < allBlobs.size(); i++){
			...
			if (allBlobs.get(i).getPerimeter() < 50)
			{
				...
				rect = adjustedPolygon.getBounds();
				for (Point p : separatingPoints){
					if (p.y == (rect.y - 1) || p.y == (rect.y + rect.height + 1))
					{
						if (Math.abs(p.x - rect.x) < 20){
	...
	public ArrayList<Point> start(String fileName){
		...
		ArrayList<Integer> valleys = findSeparator(projectionData);
		ArrayList<Point> separatingPoints = showResult(img, valleys);
	...
	public static void main(String[] args) {
		...
		Projection pj = new Projection("E:\\HisDoc project\\Gabor_filter\\projection\\case1\\");
		pj.start("patch172.png");
	}
}